...
from jaxopt._src.scipy_wrappers import make_onp_to_jnp, pytree_topology_from_example
from jaxopt._src.tree_util import tree_single_dtype
from jaxopt.tree_util import tree_add_scalar_mul, tree_l2_norm, tree_sub
...
class BFGS(base.IterativeSolver):
    ...
    def init_state(self, init_params: Any, *args, **kwargs) -> BfgsState:
        ...
        return BfgsState(
            ...
            rho=jnp.asarray(jnp.inf, dtype=dtype),
        )

    def update(self, params: Any, state: BfgsState, *args, **kwargs) -> base.OptStep:
        ...
        rho_inv = _dot(flat_y, flat_s)
        rho = jnp.reciprocal(rho_inv)

        Hy = _dot(state.H, flat_y)
        yHy = _dot(flat_y, Hy)
        c1 = (rho_inv + yHy) / (rho_inv**2)
        new_H = (
            state.H
            + c1 * jnp.outer(flat_s, flat_s)
            - rho * jnp.outer(Hy, flat_s)
            - rho * jnp.outer(flat_s, Hy)
        )
        ...
        if self.verbose:
            self.log_info(
                ...
                additional_info={
                    ...
                    "Number Linesearch Iterations": new_state.num_linesearch_iter
    ...
    def __post_init__(self):
        ...
        if self.decrease_factor is not None:
            warnings.warn("Argument decrease_factor is deprecated", DeprecationWarning)
